...
export default function Home() {
  ...
  return (
    <main className="relative h-screen w-full overflow-hidden bg-gradient-to-b from-background to-background/80 via-black/20 text-foreground flex flex-col items-center justify-center">
      {/* Title and Subtitle */}
      <div
      ...
      >
        <h1
          className={`text-6xl md:text-8xl font-bold mb-6 ${animationTriggered ? "animate-from-bottom" : "opacity-0"}`}
        >
          Code<span className="font-bold text-accent/60">Vec</span>
        </h1>
        <p
          className={`text-lg md:text-xl text-foreground/80 leading-relaxed ${animationTriggered ? "animate-from-bottom-delayed" : "opacity-0"}`}
        >
          CodeVec is a agentic multi-language tool that summarizes code at the function, module, and project
          levels—making it easy to understand, document, and onboard fast. But it's more than a summarizer.
        </p>
      </div>
      ...
      >
        <div id="playground-content" className="relative w-full h-full bg-primary/80 rounded-t-3xl overflow-hidden">
